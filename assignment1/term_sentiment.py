...
def printNewSentimentScores(dict):
	...
	for token, scores in dict.iteritems():
                positives=len([x for x in scores if x >= 0])
                negatives=len([x for x in scores if x < 0])
                if negatives !=0:
                    sentiment = (float(positives))/(float(negatives))
                else:
                    sentiment= positives
		print token.encode('utf-8').rstrip('\r\n')+"\tpositives\t" + str(positives) + "\tnegatives\t" + str(negatives) + "\tsentiment\t" + str(sentiment)
                #print token.strip()
                #print str(sentiment)
